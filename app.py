...
# Ensure page config (do this once at top of app)
st.set_page_config(page_title="PDF Chatbot & Summarizer", layout="wide", initial_sidebar_state="auto")

# Initialize session value
if "theme" not in st.session_state:
    # default to dark (set 'light' to start with light theme)
...
# Topbar container: put theme toggle to the right
# We create a small two-column header and put the button right aligned
header_cols = st.columns([1, 0.1])
with header_cols[0]:
    # leave room for your existing title if desired or render nothing here
    pass
with header_cols[1]:
    # small icon button (emoji used for simplicity)
    if st.button("🌙" if st.session_state.theme == "dark" else "☀️", key="theme_toggle_btn"):
        toggle_theme()

# Inject theme CSS (two sets). Adjust colors to taste.
# This should be placed after theme selection logic so it responds immediately.
if st.session_state.theme == "dark":
    components.html(
        ...
        html, body, [data-testid="stAppViewContainer"] {
            background: linear-gradient(180deg,#05060a,#070a12) !important;
            color: #e6eef8 !important;
        }
        .css-1v3fvcr { color: #e6eef8 !important; } /* headings */
        .stButton>button { background: #1f2937 !important; color: #f8fafc !important; }
        .stMarkdown, .stText { color: #e6eef8 !important; }
        .stAlert { background: rgba(255,255,255,0.02) !important; }
        /* card-like look for summary box */
        .summary-card { background: rgba(255,255,255,0.03); padding: 16px; border-radius: 12px; color: #dbeafe; }
        /* adjust chat bubbles if you used default st.chat_message selectors */
        .stChatMessage { color: #e6eef8; }
        </style>
        """,
        height=0,
    )
    ...
    components.html(
        ...
        .css-1v3fvcr { color: #0b1220 !important; }
        .stButton>button { background: #f6e7c4 !important; color: #1f2937 !important; }
        .stMarkdown, .stText { color: #0b1220 !important; }
        .stAlert { background: rgba(0,0,0,0.02) !important; }
        .summary-card { background: #f3f6ff; padding: 16px; border-radius: 12px; color: #0b1726; }
        .stChatMessage { color: #0b1220; }
        </style>
        """,
        height=0,
    )
        ...
        with st.chat_message("assistant"):
            with st.spinner("Retrieving relevant context and generating answer..."):
                try:
                    ...
                    relevant_chunks = [st.session_state.session_data["chunks"][i] for i, c in zip(top_indices, relevant_chunks)]
